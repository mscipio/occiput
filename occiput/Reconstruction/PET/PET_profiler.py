...
class ReconstructionProfiler(): 
    ...
    def rec_uncompress(self): 
        T = self.tac(); self.T_rec['T13_uncompress'] += T
    def rec_get_subset_attenuation(self): 
        T = self.tac(); self.T_rec['T18_get_subset_attenuation'] += T
    def rec_get_subset_sensitivity(self): 
        T = self.tac(); self.T_rec['T17_get_subset_sensitivity'] += T
    def rec_get_subset_randoms(self): 
        T = self.tac(); self.T_rec['T15_get_subset_randoms'] += T
    def rec_get_subset_prompts(self): 
        T = self.tac(); self.T_rec['T16_get_subset_prompts'] += T
    def rec_get_subset_scatter(self): 
        T = self.tac(); self.T_rec['T14_get_subset_scatter'] += T
    def rec_compose_various(self): 
        T = self.tac(); self.T_rec['T21_compose_various'] += T
    def rec_compose_randoms(self): 
        T = self.tac(); self.T_rec['T19_compose_randoms'] += T
    def rec_compose_scatter(self): 
        T = self.tac(); self.T_rec['T20_compose_scatter'] += T
    def rec_backprojection_norm_total(self): 
        T = self.tac(); self.T_rec['T22_backprojection_norm_total'] += T
    def rec_backprojection_activity_total(self): 
        T = self.tac(); self.T_rec['T12_backprojection_activity_total'] += T
    def rec_projection_activity_total(self): 
        T = self.tac(); self.T_rec['T11_projection_activity_total'] += T
    def rec_update(self): 
        T = self.tac(); self.T_rec['T23_update'] += T
        
    def rec_project_projection(self): 
        T = self.tac(); self.T_project['t1_project_total'] = T
    def rec_project_make_continuous(self): 
        T = self.tac(); self.T_project['t2_make_continuous'] = T
    def rec_project_get_subset_sparsity(self): 
        T = self.tac(); self.T_project['t3_get_subset_sparsity'] = T
    def rec_project_scale(self): 
        T = self.tac(); self.T_project['t4_scale'] = T
    def rec_project_wrap(self): 
        T = self.tac(); self.T_project['t5_wrap'] = T
    def rec_project_get_angles(self): 
        T = self.tac(); self.T_project['t6_get_angles'] = T
    def rec_project_exponentiate(self): 
        T = self.tac(); self.T_project['t7_exponentiate'] = T

        
    def rec_backpro_backprojection(self): 
        T = self.tac(); self.T_backpro['t1_backproject_total'] = T
    def rec_backpro_get_subset(self): 
        T = self.tac(); self.T_backpro['t2_get_subset'] = T
    def rec_backpro_get_subset_data(self): 
        T = self.tac(); self.T_backpro['t3_get_data_subset'] = T
    def rec_backpro_get_subset_sparsity(self): 
        T = self.tac(); self.T_backpro['t4_get_subset_sparsity'] = T
    def rec_backpro_scale(self): 
        T = self.tac(); self.T_backpro['t5_scale'] = T
    def rec_backpro_wrap(self): 
        T = self.tac(); self.T_backpro['t6_wrap'] = T      
    def rec_backpro_get_angles(self): 
        T = self.tac(); self.T_backpro['t7_get_angles'] = T
